...
if [ -z "${DRUPAL_PATH}" ] || [ ! -d "${DRUPAL_PATH}/sites" ]; then
...
chown -R $DRUPAL_USER:$APACHE_GROUP $DRUPAL_PATH

echo "Changing permissions of all directories inside \"${DRUPAL_PATH}\" to \"rwxr-x---\"..."
find . -type d -exec chmod u=rwx,g=rx,o= '{}' \;
echo "Changing permissions of cache directorie inside \"${DRUPAL_PATH}\" to \"g+w\"..."
chmod -R g+w cache
